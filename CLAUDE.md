...
- Tab order is **Home · Blogs · Experience · Contact** and does not change. `NativeTabs.Trigger` names are `(home)`, `blog`, `experience`, `contact` — `(home)` is deliberately a route group (transparent, maps to bare `/`); the other three are literal folders so `/blog`, `/experience`, `/contact` are real, reachable deep-link paths. (Fixed 2026-08-16: these were originally shipped as groups, which left them with no reachable path — they silently fell through to `blog/[slug].tsx`'s dynamic segment. Caught during M8 device-based deep-link verification. **2026-08-18:** Settings tab removed; appearance picker moved to the Home header dropdown.)
